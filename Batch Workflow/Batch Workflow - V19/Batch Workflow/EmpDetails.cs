    ...
    class EmpDetails
    {
        public void empdetails_list(DataTable dta)
        {
            ...
            try
            {
                ...
                cmd.CommandText = "select * from dbo.tbl_emp_details with(nolock) where Process = 'Batch Checking' and IsDeleted = 0 union all select * from dbo.tbl_emp_details with(nolock) where empname in ('Gautami Kamath','Chaitali Dutta','Mahek Bhalla') and IsDeleted = 0 order by EmpName";
        ...
        public void empdetails_searchby_passfort_list(DataTable dta)
        {
            string connectionstringtxt = "Data Source=A20-CB-DBSE01P;Initial Catalog=DRD;User ID=DRDUsers;Password=24252425";
            SqlCommand cmd = new SqlCommand();
            SqlConnection conn = new SqlConnection();
            ...
            try
            {
                ...
                cmd.Parameters.Clear();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "select * from dbo.tbl_emp_details with(nolock) where Process = 'Batch Checking' and IsDeleted = 0 union all select * from dbo.tbl_emp_details with(nolock) where empname = 'Gautami Kamath' union all select * from dbo.tbl_emp_details with(nolock) where empname in ('Reshma Nair','Priyanshu Yadav','Pranay Kamble') and IsDeleted = 0  order by EmpName";
                sda.SelectCommand = cmd;
                dt = dta;
                sda.Fill(dta);
            }
